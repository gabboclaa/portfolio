...
function useScramble(finalText: string, startDelay = 500, duration = 1400) {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(startTimeout); cancelAnimationFrame(rafId); };
  ...
  return (
    <>
      <style>{`
        ...
        .hero-badge  { animation: slideUp 0.6s ease 1.6s both; }
        .hero-desc   { animation: slideUp 0.6s ease 2.2s both; }
        .hero-links  { animation: slideUp 0.6s ease 2.5s both; }
        .hero-scroll { animation: slideUp 0.6s ease 3s both; }
        .typed-cursor { display:inline-block; width:2px; height:0.8em; background:#bd864b; vertical-align:middle; margin-left:2px; animation: blink 1s step-end infinite; }
      `}</style>
